...
class Client implements LoggerAwareInterface, Stringable
{
    ...
    protected function performHandshake(Uri $host_uri): Response
    {
        ...
        // connection set deflate
        $permessageDeflate = in_array('permessage-deflate', $response->getHeader('sec-websocket-extensions'));
        $this->connection->setDeflate($permessageDeflate);
    ...
    public static function echoBinary($strDec) {
        $strHex = '';
        for ($i = 0; $i < strlen($strDec); $i++) {
            if ($i > 0) $strHex .= ' ';
            $strHex .= '0x' . substr('0' . dechex(ord($strDec[$i])), -2);
        }
        return $strHex;
    }
}
