...
class FrameHandler implements LoggerAwareInterface, Stringable
{
    ...
    public function setDeflate(bool $enableDeflate): void
    {
        ...
        $this->logger->debug("[frame-handler] Set permessage-deflate " . ($enableDeflate ? "'Enabled'" : "'Disabled'"));
    }
    ...
    public function pull(): Frame
    {
        ...
        $this->logger->debug('[frame-handler] Recive byte_1: 0b' . substr('0000000' . decbin($byte_1), -8) . ' byte_2: 0b' . substr('0000000' . decbin($byte_2), -8));
        $final = (bool)($byte_1 & 0b10000000); // Final fragment marker.
        $rsv = $byte_1 & 0b01110000; // Unused bits, ignore

        // get rsv1 bit from any deflate frame
        if ($rsv === 64) {
            $this->enableDeflate = true;
            $this->logger->debug("[frame-handler] Recive data is 'Deflated'");
        }
        ...
        if ($masked) {
            ...
            $this->logger->debug("[frame-handler] Recive Mask [ " . Client::echoBinary($masking_key) . " ]");
        }
    ...
    public function push(Frame $frame,int $frameCount, bool|null $masked = null): int
    {
        $final = $frame->isFinal();
        ...
        // set first deflate frame rsv1 bit
        if ($this->enableDeflate && !$frameCount) {
            $byte_1 = $final ? 0b11000000 : 0b01000000;
            $this->logger->debug("[frame-handler] Send data is 'Deflated'");
        } else {
        ...
        if ($this->pushMasked) {
            ...
            $data .= $mask;
            $this->logger->debug("[frame-handler] Send Mask [ " . Client::echoBinary($mask) . " ]");
